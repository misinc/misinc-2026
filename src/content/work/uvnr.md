---
title: "United Voices for Newcomer Rights"
client: "United Voices for Newcomer Rights"
summary: "An Albuquerque nonprofit supporting refugees and immigrants, where the site has to serve the people it helps and the people who fund it."
category: nonprofit
image: /img/work/uvnr.jpg
imageAlt: "United Voices for Newcomer Rights nonprofit website"
projectUrl: https://www.uvnr.org
year: 2023
order: 11
services: [web-design, website-development]
platforms: [webflow]
seo:
  title: "United Voices for Newcomer Rights Website | MIS, Inc. Portfolio"
  description: "How MIS, Inc. built the website for United Voices for Newcomer Rights, an Albuquerque nonprofit supporting refugee and immigrant communities."
---

United Voices for Newcomer Rights is a community-led organisation founded in Albuquerque in 2019. It works on the immediate needs of newcomer communities and on the longer project of systemic change — educational, economic, cultural, and health initiatives, run in partnership with the communities themselves rather than on their behalf.

## Two audiences with nothing in common

Nonprofit sites usually carry a split, and this one is wider than most.

A newcomer arriving at the site needs practical help now: housing, employment, schools, and knowing what their rights are. A donor, funder, or volunteer needs to understand the mission and see that the organisation is credible.

Those two readers want different things, arrive through different routes, and in many cases read in different languages. The site keeps the resources route short and unmistakable — someone looking for housing information should never have to read the case for support to find it — while the mission and giving material has its own space rather than being wrapped around everything else.

## Programmes, named as they are

The programme names are specific and worth preserving: Tea Circle, the Refugee and Immigrant Well-Being Project, Pathways Mentoring, Youth Empowerment, Interpretation and Translation, and Systemic Change.

There is always a temptation to rename this kind of thing into something more legible to funders. We left them, because the communities using them know them by these names, and legibility to a grant officer is a poor trade for legibility to the people the organisation exists for.

Built on [Webflow](/platforms/webflow), so a small team can post news, newsletters, job openings, and updated resources without a developer in the loop.
